...
1. **Нужно ли в Phase 7 уже подключать pino к stdout, или достаточно фабрики + точечной замены?**
   ...
   - Рекомендация: в Phase 7 ввести фабрику + DI-инъекцию + `child({runId})` и заменить ровно те вызовы, которые являются логами (не контрактным summary). Если таких отдельных лог-вызовов сейчас нет, Phase 7 поставляет **substrate + проводку**, а перевод summary остаётся PROG (Phase 11). Это сохраняет «no behavioral change». Планировщику стоит явно зафиксировать этот scope-разрез в задачах.
...
2. **`stdout` против `stderr` для будущих прогресс-логов.**
   ...
   - Рекомендация: сделать `destination` фабрики конфигурируемым (для тестов и будущего выбора канала); в Phase 7 не вводить конкурирующих stdout-строк.
